...
### 9.1 Deferred (may revisit in v0.6+)
...
### 9.2 Anti-features — permanent NO (project-wide guard, carry over to every future BRIEF)

These are explicitly OUT of sshc's scope, not just v0.5. They protect
the positioning as **personal / homelab SSH cockpit** — fzf-friendlier
than raw shell snippets, storm-safer than inline-editing tools, lighter
than full SSH GUIs.

1. **Self-built SSH client** — no PTY emulation, no key exchange, no
   crypto. `std::process::Command("ssh")` is the right answer; the
   value-add is the picker, not the transport.
2. **Secret / key management** — passwords, keyfiles, agent forwarding
   policy are 1Password / ssh-agent / macOS Keychain territory. sshc
   reads `IdentityFile` paths and that's it.
3. **Team-shared catalogs, permissions, audit logs** — different
   product. The moment "who can see this host" matters, the picker is
   not the right tool.
4. **Web UI / daemon / always-on background process** — the entire
   appeal is "type, Enter, you're in." A daemon kills that.
5. **Complete OpenSSH `config(5)` parser** — over-investment. Pair a
   conservative line-oriented parser with `ssh -G <alias>` for the
   edge cases. OpenSSH is the source of truth; sshc is a curator.

When a feature request hints at any of the five above, redirect or
decline. Don't apologize for the small surface area — it's the point.
